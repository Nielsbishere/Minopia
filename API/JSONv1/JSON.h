 ...
 *
 *     This version of JSON formatting was not for development; JSONv1's API is buggy when using tabs, spaces and enters in between anything.
 *     Also this version doesn't support entering lists, scientific notation for floating points and \n \t etc didn't get parsed.
 */
...
public:
	...
	static JSON *parseJSON(string s) {
		...
		//Instead of removing enters and tabs, just loop to check the first and the last character and set it to ""
	...
	double _double(string s) {
		Query qr = prop(s);
		if (qr.getType() != "Double" || qr.getValue() == nullptr)
			return 0;
	...
	int getIndex(string s){
		for(int i=0;i<getKeys();i++)if(getKey(i)==s)return i;
		return -1;
	}
