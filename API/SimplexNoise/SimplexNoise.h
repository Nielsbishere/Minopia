...
public:
	SimplexNoise() :
			grad3 { Grad(1, 1, 0), Grad(-1, 1, 0), Grad(1, -1, 0), Grad(-1, -1,
					0), Grad(1, 0, 1), Grad(-1, 0, 1), Grad(1, 0, -1), Grad(-1,
					0, -1), Grad(0, 1, 1), Grad(0, -1, 1), Grad(0, 1, -1), Grad(
					0, -1, -1) }, grad4 { Grad(0, 1, 1, 1), Grad(0, 1, 1, -1),
					...
					Grad(-1, 1, -1, 0), Grad(-1, -1, 1, 0), Grad(-1, -1, -1, 0) }, p {
		...
		for (int i = 0; i < 512; i++) {
			perm[i] = p[i & 255];
			permMod12[i] = perm[i] % 12;//TODO: this can maybe be a little bit of trouble, if it's not converted
		}
		F2 = 0.5 * (sqrt(3.0) - 1.0);
		G2 = (3.0 - sqrt(3.0)) / 6.0;
		F4 = (sqrt(5.0) - 1.0) / 4.0;
	...
	double noise(double x, double scale, int octaves, double persistance,
			double lacunarity) {
		double freq = 1, amp = 1;
		double y = 0;
	...
	double noise(double x, double y, double scale, int octaves,
			double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double z = 0;
		for (int i = 0; i < octaves; i++) {
	...
	double tnoise(double x, double y, double scale, int octaves,
				double persistance, double lacunarity) {
			double freq = 1, amp = 1;
			double z = 0;
			for (int i = 0; i < octaves; i++) {
	...
	double noise(double x, double y, double z, double scale, int octaves,
			double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double w = 0;
		for (int i = 0; i < octaves; i++) {
	...
	double tnoise(double x, double y, double z, double scale, int octaves,
				double persistance, double lacunarity) {
			double freq = 1, amp = 1;
			double w = 0;
			for (int i = 0; i < octaves; i++) {
	...
	double noise(double x, double y, double z, double a, double scale, int octaves,
			double persistance, double lacunarity) {
		double freq = 1, amp = 1;
		double w = 0;
		for (int i = 0; i < octaves; i++) {
	...
	double tnoise(double x, double y, double z, double a, double scale, int octaves,
				double persistance, double lacunarity) {
			double freq = 1, amp = 1;
			double w = 0;
			for (int i = 0; i < octaves; i++) {
